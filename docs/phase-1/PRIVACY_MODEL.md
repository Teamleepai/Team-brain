...
| `error_flag` | Indefinite | Repoint `flagged_by` at a reserved tombstone user | Learning signal for Phase 3. Retains value once detached from who flagged it. See §6.3 item 3. |
| **Logs, metrics, traces** | 30 days for application logs, 90 days for metrics and traces | Not individually addressable | `OBSERVABILITY.md` requires a clear deletion policy for anything containing PII, and T8 in `THREAT_MODEL.md` establishes that despite the redaction rule some personal data will reach logs. Short retention is the control that bounds a leak we cannot fully prevent. Now recorded in `DATA_MODEL.md` §12. |
...
| **Objection / restriction** | Add the identifier to a suppression list checked at the normalizer, so future content involving that person is not ingested | **Resolved:** `DATA_MODEL.md` §5 now defines `ingestion_suppression`, checked before any episodic write. A suppression list is the only mechanism that makes objection meaningful, since deletion without it means re-ingestion on the next run. Identifiers are stored hashed, because a plaintext list of people who asked not to be processed is itself sensitive personal data. |
...
- `episodic_record` rows where the subject is the sole or a named participant: reduced to a content tombstone, with `payload` and `participants` cleared and the row and its links retained (see item 2 below for why the row cannot simply be deleted).
...
1. **`audit_record` has `UPDATE` revoked (`DATA_MODEL.md` §9) and redaction requires `UPDATE`. — RESOLVED.** This was a direct contradiction between §9 and §12 of the data model. Three options were weighed: (a) a `SECURITY DEFINER` function owned by a privileged role as the sole permitted mutation path, itself writing an audit record of the redaction; (b) storing personal content in a separate `audit_payload` table that permits deletion, leaving `audit_record` immutable and content-free; (c) accepting that redaction runs as the service role outside the privilege model — which is how this usually happens and is the wrong answer, because it means the append-only property was never real.

    **Option (a) is implemented**, as `redact_audit_pii` in `DATA_MODEL.md` §9. It keeps append-only as the default, makes each exception self-documenting, and audits the redaction itself. Option (b) remains the cleaner end state and is worth revisiting if redaction volume ever makes the function a bottleneck, but it is a larger schema change for no Phase 1 benefit.
2. **`episodic_record` hard deletion collides with `semantic_link`'s foreign key and with the no-orphans rule. — RESOLVED.** `DATA_MODEL.md` §12 said hard delete; §7 requires every `semantic_record` to retain at least one `semantic_link` to an `episodic_record`, enforced by a commit-time trigger. Deleting the last episodic source of a semantic record therefore either failed on the constraint or orphaned the record, and both `ARCHITECTURE.md` §6 and `DATA_MODEL.md` §1 forbid the second. Worth noting *why* the orphan is the worse outcome: a fact with no provenance cannot be explained, so permitting it would trade a privacy obligation for an explainability violation rather than satisfying either.

    Erasure now replaces the episodic row with a **content tombstone** retaining `id`, `org_id`, `source_connection_id`, `kind`, `occurred_at`, and a redaction marker, with `payload` and `participants` cleared. Links stay valid, no orphan is created, and no content survives. `DATA_MODEL.md` §12 has been amended accordingly. One refinement on the original proposal: `content_hash` is **retained**, because a hash is not content and keeping it preserves change-detection on any future re-fetch of the same source identifier.
3. **`error_flag.flagged_by` is a non-null foreign key to `app_user`, so it cannot be anonymized by nulling. — RESOLVED.** Either the column becomes nullable, or anonymization repoints it to a permanent tombstone user row. **The tombstone was chosen** and `DATA_MODEL.md` §10 now says so: it keeps the constraint meaningful and makes the anonymization visible in the data rather than inferable from an absence, which is the same reasoning that makes redaction elsewhere write a marker instead of `NULL`.
...
4. **`DATA_MODEL.md` amendments this document required — ALL FOUR APPLIED.** Each was a real defect rather than a stylistic preference: `agent_proposal` missing from the retention table (now present, 7 years, matching `audit_record`); the `audit_record` `REVOKE UPDATE` versus redaction contradiction (§6.3 item 1, resolved via `redact_audit_pii`); episodic "hard delete" versus `semantic_link`'s foreign key and the no-orphans trigger (§6.3 item 2, resolved via content tombstone); `error_flag.flagged_by` non-null versus anonymization (§6.3 item 3, column is now nullable). A retention row for logs, metrics, and traces was added at the same time, which `OBSERVABILITY.md` requires and no document had covered.
