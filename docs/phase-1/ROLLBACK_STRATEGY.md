...
**A bad fact with no corrected successor cannot be marked invalid.** The `supersession_complete` check constraint requires `superseded_by` and `valid_until` to be both null or both non-null. That constraint is correct for its intended purpose — it prevents the half-state where a record reads as currently valid while pointing forward. But it also means there is no way to express "this record is garbage and there is no replacement," which is the common case when a model hallucinated a commitment that was never made. The options are a tombstone successor record (a `retraction`-kind row whose statement is "the preceding claim was retracted") or relaxing the constraint to allow `valid_until` without `superseded_by`, paired with a non-null `invalidation_reason`. The tombstone preserves the constraint and keeps every invalid record pointing at an explanation, at the cost of rows that exist only to say something does not. Recommended, and flagged in §8.

**`unique (org_id, dedupe_key)` blocks the corrected successor.** If `dedupe_key` is a normalized identity for a claim, then a corrected version of the same claim computes the *same* key, and inserting it violates the unique constraint while the superseded row still holds that key. Supersession and a global unique dedupe key are in direct conflict. The fix is that the constraint must be a partial unique index scoped to live records, `unique (org_id, dedupe_key) where superseded_by is null`, mirroring the pattern already used for `one_active_prompt_per_agent` and for the active-records index. This is a defect in `DATA_MODEL.md` §7 rather than a Phase 1 choice, and it must be corrected before the first migration, because after data exists the correction is itself a migration against a populated unique index.
...
**Two schema defects block the distillation rollback story and must be fixed before the first migration.** Both are in §2. `unique (org_id, dedupe_key)` on `semantic_record` conflicts with supersession and must become a partial unique index scoped to live records. And `supersession_complete` makes it impossible to invalidate a record with no corrected successor, which is the common case for a hallucinated fact. These are cheap to fix now and become migrations against populated tables later.
