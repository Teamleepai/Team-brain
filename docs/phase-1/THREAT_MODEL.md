...
| **T17** | **Forged Zoom webhook payloads** | `ADR-0008` makes increment 1b push-based: Zoom `POST`s transcript-ready events to a public endpoint. An unverified or weakly verified endpoint lets anyone `POST` a fabricated event and inject an arbitrary transcript straight into episodic memory. | Worse than T11's ordinary injection surface, and worth being precise about why: with an email body the attacker controls *content inside* a record whose provenance is genuine. Here the attacker controls **the entire record, including its provenance** — the claimed meeting, participants, and timestamp. A fabricated transcript can therefore manufacture a commitment attributed to a real person at a real time, which then distils into a semantic record that looks perfectly sourced. | Zoom signature verification on every request, rejecting before parsing; the endpoint accepts only the event envelope and fetches the transcript itself from the Zoom download URL rather than trusting inline content; normalizer schema validation unchanged; `source_connection_id` fixed per connection so a forged event cannot claim a different source. | Medium, and this is the new attack surface `ADR-0008` introduces. Signature verification is the whole defense and it must be constant-time and fail-closed. Note the second mitigation is the stronger one: because the adapter re-fetches from Zoom using an authenticated download token, a forged event with no valid recording behind it dies at the fetch rather than at the parse. |
| **T18** | Silent gaps in webhook-delivered transcripts | Also from `ADR-0008`. Pull-based sources are protected by a watermark: a missed page is re-fetched next cycle. A webhook has no cursor, so if Zoom exhausts its delivery retries the event is simply gone, and nothing in the system knows a meeting existed. | A confidently incomplete briefing, with the same shape as T15 but no mechanism to self-correct. Commitments made in an un-ingested meeting are invisible rather than late. | **Requires a reconciliation sweep** — a periodic authenticated `GET` over recent recordings, comparing against ingested `source_id`s and backfilling gaps. Recorded as technical debt in `ADR-0008`. | **Unmitigated until the sweep is built, and the sweep is not optional.** This is the one place where `ADR-0008`'s push model is strictly weaker than the pull model it replaces, and shipping 1b without reconciliation would knowingly book the failure mode `FAILURE_MODES.md` grades most dangerous. |
